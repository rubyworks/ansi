...
This release introduced a preliminary rendition of a Diff class
for getting coloreized comparions of strings and other objects.
It's not officially supported yet, so this is only a point release.

Changes:

* Added Diff class for colorized comparisons.
* Fixed minor issue with Columns format block --col argument before row.
